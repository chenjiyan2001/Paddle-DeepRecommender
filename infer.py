...
def create_data_loader(config, place):
    ...
    train_file_list = [os.path.join(train_data_dir, x) for x in os.listdir(train_data_dir)]
    ...
    test_file_list = [os.path.join(test_data_dir, x) for x in os.listdir(test_data_dir)]
    logger.info("reader path:{}".format(reader_path))
    reader_class = import_module(reader_path)
    dataset = reader_class.RecDataset(train_file_list, config=config, test_list=test_file_list)
    loader = DataLoader(
